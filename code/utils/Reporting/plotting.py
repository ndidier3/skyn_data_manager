...
from utils.Stats.get_feature_importances import get_feature_importances
import numpy as np
from sklearn.metrics import plot_roc_curve
from sklearn.tree import plot_tree


def plot_column(df, plot_folder, subid, condition, sub_condition, y_variable, time_variable, xlabel="Time (hours)"):
      ylabel = y_variable    
      ...
      if not os.path.exists(folder):
            os.mkdir(folder)
      full_path = f'{folder}/{y_variable} - {subid} - {condition}{sub_condition}.png'
      fig.savefig(full_path)
      plt.close()
      return full_path

def plot_TAC_curve(df, plot_folder, subid, condition, sub_condition, tac_variable, time_variable, ylabel="TAC ug/L", xlabel="Time (hours)"):
      ...
      full_path = f'{folder}/{tac_variable} - {subid} - {condition}{sub_condition}.png'
      fig.savefig(full_path)
      plt.close()
      return full_path

def plot_overlaid_TAC_curves(df, plot_folder, subid, condition, sub_condition, tac_variables, time_variable, plot_name, ylabel="TAC ug/L", xlabel="Time (hours)"):
      ...
      full_path = f'{folder}/{plot_name} - {subid} - {condition}{sub_condition}.png'
      fig.savefig(full_path)
      plt.close()
      return full_path

def plot_overlaid_with_normalization(df_prior, plot_folder, subid, condition, sub_condition, variables, time_variable, plot_name):
      df = df_prior.copy()
...
def plot_tac_and_temp(df, plot_folder, subid, condition, sub_condition, tac_variable, temp_variable, time_variable, plot_title = "TAC and Temperature"):
      fig, ax = plt.subplots(figsize=(16, 7))
      df.plot(x = time_variable, y = tac_variable, ax = ax)
      df.plot(x = time_variable, y = temp_variable, ax = ax, secondary_y = True)
      fig.suptitle(plot_title, fontsize=14)
      path = f'{plot_folder}/{subid}/{condition}/tac_and_temp_plot_{subid}_{condition}{sub_condition}.png'
      fig.savefig(path)
      plt.close()

      return path

def plot_temp_cleaning(df, plot_folder, subid, condition, sub_condition, temp_variable, time_variable, add_color=False, plot_title = "Temperature Assessment"):
      if add_color:
      ...
      path=f'{plot_folder}/{subid}/{condition}/{subid}_{condition}{sub_condition}_temp_cleaning.png'
      plt.tight_layout()
      plt.savefig(path, bbox_inches='tight')
      plt.close()
      return path

def plot_cropping(data, beginning_timestamp, end_timestamp, subid, condition, sub_condition, directory, cutoff, add_color = False, title="Raw Data Cropping"):
      if add_color:
      ...
      path = f'{directory}/{subid}_{condition}{sub_condition}_cropping.png'
      plt.tight_layout()
      plt.savefig(path, bbox_inches='tight')
      plt.close()
      return path
...
def plot_cleaning_comparison(occasion, df, df_raw, time_variable, add_color = False, title = "TAC Signal Cleaning", size = (16, 7), legend = True, snip = None):
      ...
      filename = f'cleaning - {occasion.subid} - {occasion.condition}{occasion.sub_condition}'
      if snip:
...
def plot_smoothed_curve(df, plot_folder, subid, condition, sub_condition, time_variable, peak, baseline_cutoff, curve_begins, curve_ends, title = "TAC Curve"):
      ...
      path = f'{plot_folder}/{subid}/{condition}/{subid}_{condition}{sub_condition}_smoothed_curve.png'
      plt.tight_layout()
      plt.savefig(path, bbox_inches='tight')
      plt.close()
      return path
