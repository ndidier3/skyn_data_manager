...
def LR_CV(features, predictors, grouping_column_name, label_column_name, 
  ...
  #find optimal LR settings
  lr_optimal = create_optimal_lr(cv_method=GroupKFold(n_splits=n_splits))
  #train model using those settings
  lr_optimal.fit(X, y, groups=split_groupings)
  #use model to make predictions and probabilities
  predictions = cross_val_predict(lr_optimal.best_estimator_, X, y, cv=GroupKFold(n_splits=n_splits), groups=split_groupings, method='predict')
  probabilities = cross_val_predict(lr_optimal.best_estimator_, X, y, cv=GroupKFold(n_splits=n_splits), groups=split_groupings, method='predict_proba')
...
def LR_CV_alc_vs_non(features, predictors = ['auc_per_hour', 'curve_auc', 'rise_rate', 'fall_duration', 
  ...
  cv_stats, cv_results, incorrect, correct, predictors, n_splits, lr_optimal_estimator_ = LR_CV(
      features, predictors, grouping_column_name='subid', label_column_name='condition', 
      relabeling_dict = {'Alc': 1, 'Non': 0}, #Converting text into numbers
      filter_dict = {'valid_occasion': [0]}
    )
  return cv_stats, cv_results, incorrect, correct, predictors, n_splits, lr_optimal_estimator_


def LR_CV_worn_vs_removed(features, predictors = ['temp', 'temp_pre_neighbors', 'temp_post_neighbors', 'temp_diff',
  ...
  cv_stats, cv_results, incorrect, correct, predictors, n_splits, lr_optimal_estimator_ = LR_CV(
      features, predictors, grouping_column_name='subid', label_column_name='condition', 
      relabeling_dict = {'Worn': 1, 'Removed': 0} #Converting text into numbers
    )
  return cv_stats, cv_results, incorrect, correct, predictors, n_splits, lr_optimal_estimator_
