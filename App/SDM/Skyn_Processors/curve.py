...
class Curve:
  def __init__(self, df: pd.DataFrame, subid, dataset_identifier, curve_id, curve_start, curve_end, curve_count, curve_threshold, flag_selections = {}, periphery_buffer_before = 2, periphery_buffer_after = 2, TAC_column = 'TAC'):
    ...
    self.curve_quality_features = {
      ...
      'unimputed_gap_duration_CURVE': (self.curve['gap'] == 1).sum() / 60 if self.TAC_column == 'TAC_pre_imputation' else get_unimputed_gap_duration(self.curve),
      'unimputed_gap_percent_CURVE': (self.curve['gap'] == 1).sum() / len(self.curve) if self.TAC_column == 'TAC_pre_imputation' else get_unimputed_gap_percent(self.curve),
      ...
      'unimputed_non_wear_duration_CURVE': (self.curve['non_wear'] == 1).sum() / 60 if self.TAC_column == 'TAC_pre_imputation' else get_unimputed_non_wear_duration(self.curve),
      'unimputed_non_wear_percent_CURVE': (self.curve['non_wear'] == 1).sum() / len(self.curve) if self.TAC_column == 'TAC_pre_imputation' else get_unimputed_non_wear_percent(self.curve),
    ...
    self.periphery_quality_features = {
      ...
      'unimputed_gap_duration_PERIPHERY': (self.periphery['gap'] == 1).sum() / 60 if self.TAC_column == 'TAC_pre_imputation' else get_unimputed_gap_duration(self.periphery),
      'unimputed_gap_percent_PERIPHERY': (self.periphery['gap'] == 1).sum() / len(self.periphery) if self.TAC_column == 'TAC_pre_imputation' else get_unimputed_gap_percent(self.periphery),
      'total_gap_duration_PERIPHERY': (self.periphery['gap'] == 1).sum() / 60,
      'total_gap_percent_PERIPHERY': (self.periphery['gap'] == 1).sum() / len(self.periphery),
      ...
      'unimputed_non_wear_duration_PERIPHERY': (self.periphery['non_wear'] == 1).sum() / 60 if self.TAC_column == 'TAC_pre_imputation' else get_unimputed_non_wear_duration(self.periphery),
      'unimputed_non_wear_percent_PERIPHERY': (self.periphery['non_wear'] == 1).sum() / len(self.periphery) if self.TAC_column == 'TAC_pre_imputation' else get_unimputed_non_wear_percent(self.periphery),
      'total_non_wear_duration_PERIPHERY': (self.periphery['non_wear'] == 1).sum() / 60,
      'total_non_wear_percent_PERIPHERY': (self.periphery['non_wear'] == 1).sum() / len(self.periphery),
    ...
    self.region_quality_features = {
      ...
      'unimputed_gap_duration_REGION': (self.region['gap'] == 1).sum() / 60 if self.TAC_column == 'TAC_pre_imputation' else get_unimputed_gap_duration(self.region),
      'unimputed_gap_percent_REGION': (self.region['gap'] == 1).sum() / len(self.region) if self.TAC_column == 'TAC_pre_imputation' else get_unimputed_gap_percent(self.region),
      ...
      'unimputed_non_wear_duration_REGION': (self.region['non_wear'] == 1).sum() / 60 if self.TAC_column == 'TAC_pre_imputation' else get_unimputed_non_wear_duration(self.region),
      'unimputed_non_wear_percent_REGION': (self.region['non_wear'] == 1).sum() / len(self.region) if self.TAC_column == 'TAC_pre_imputation' else get_unimputed_non_wear_percent(self.region),
