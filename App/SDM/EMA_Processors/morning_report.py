import pandas as pd
import traceback
from SDM.Visualization.device_nonwear import create_histogram_self_reported_nonwear
...
class MorningReport:
  def __init__(self, path):
    ...
    self.device_nonwear_columns = ['braceoff1', 'braceon1', 'braceoff2_1', 'braceon2_1', 'braceoff2_2', 'braceon2_2', 'braceoff3_1', 'braceon3_1', 'braceoff3_2', 'braceon3_2', 'braceoff3_3', 'braceon3_3']
    self.includes_bracelet_off = all([col in self.morning_data.columns for col in self.device_nonwear_columns])
  ...
  def calculate_non_wear(self):
    self.morning_data[self.device_nonwear_columns] = self.morning_data[self.device_nonwear_columns].apply(pd.to_datetime, format='%H:%M', errors='coerce')
    self.device_off_duration_columns = []
    for idx in range(0, len(self.device_nonwear_columns), 2):
      new_col_name = self.device_nonwear_columns[idx] + '_duration_hrs'
      self.device_off_duration_columns.append(new_col_name)
      self.morning_data[new_col_name] = self.morning_data[self.device_nonwear_columns[idx+1]] - self.morning_data[self.device_nonwear_columns[idx]]
    ...
    self.morning_data['device_nonwear_duration_hrs'] = self.morning_data[self.device_off_duration_columns].sum(axis=1)

  def calculate_non_wear_duration_by_day(self):
    self.nonwear_duration_by_day = []
    for day in self.day_ids:
      day_data = self.morning_data[self.morning_data['EMADAYN']==day]
      self.nonwear_duration_by_day.append(day_data['device_nonwear_duration_hrs'].mean())
    self.day_level_metrics['nonwear_duration'] = self.nonwear_duration_by_day

  def calculate_non_wear_count_by_day(self):
    self.nonwear_count_by_day = []
    for day in self.day_ids:
      day_data = self.morning_data[self.morning_data['EMADAYN']==day]
      ...
      self.nonwear_count_by_day.append(average_non_wear_count)
    self.day_level_metrics['nonwear_count'] = self.nonwear_count_by_day

  def run_quality_checks(self):
    device_off_columns = [col for col in self.device_nonwear_columns if 'off' in col]
    ...
    self.morning_data['flag_nonwear_less_10_min'] = self.morning_data['device_nonwear_duration_hrs'].apply(lambda x: 1 if (x < (10/60)) and (x > 0) else 0)
    self.morning_data['flag_nonwear_over_half_day'] = self.morning_data['device_nonwear_duration_hrs'].apply(lambda x: 1 if x > 12 else 0)
    
    self.count_device_off_time_within_30_min_of_survey_by_day = []
    self.count_nonwear_less_10_min_by_day = []
    self.count_nonwear_over_half_day_by_day = []
    for day in self.day_ids:
      day_data = self.morning_data[self.morning_data['EMADAYN']==day]
      ...
      self.count_nonwear_less_10_min_by_day.append(day_data['flag_nonwear_less_10_min'].sum())
      self.count_nonwear_over_half_day_by_day.append(day_data['flag_nonwear_over_half_day'].sum())
    ...
    self.day_level_metrics['count_nonwear_less_10_min'] = self.count_nonwear_less_10_min_by_day
    self.day_level_metrics['count_nonwear_over_half_day'] = self.count_nonwear_over_half_day_by_day
  ...
  def plot_non_wear_duration_by_day(self, out):
    try:
      create_histogram_self_reported_nonwear(self.day_ids, self.nonwear_duration_by_day, out, version='Duration')
    except Exception:
      if len(self.nonwear_duration_by_day) == 0:
        print('Non-wear data not found.')
      print(traceback.format_exc())

  def plot_non_wear_count_by_day(self, out):
    try:
      create_histogram_self_reported_nonwear(self.day_ids, self.nonwear_count_by_day, out, version='Count')
    except Exception:
      if len(self.nonwear_count_by_day) == 0:
        print('Non-wear data not found.')
      print(traceback.format_exc())
